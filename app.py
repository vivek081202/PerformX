...
<style>
    ...
    /* Sticker label */
    .field-label { font-size:0.82rem; font-weight:600; color:#CBD5E1 !important;; margin-bottom:0.2rem; }
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown(""" <div style='font-size:1.8rem;font-weight:700;color:#F9FAFB;'>
    PerformX Tracker
    </div>""", unsafe_allow_html=True)
    st.markdown("<small style='opacity:0.7'>Universal Performance Framework</small>", unsafe_allow_html=True)
    st.markdown("""
    <div style='margin-bottom:0.2rem;'>

    <div style='font-size:0.95rem;color:#60A5FA;margin-top:0.3rem;'>
    A structured framework to measure, reflect, and communicate performance
    </div>

    <div style='margin-top:0.8rem;font-size:0.88rem;color:#CBD5E1;line-height:1.5;'>
    ...
    st.markdown(f"**Review Progress**")
    st.progress(pct / 100)
    st.markdown(f"<small>{pct}% of metrics scored</small>", unsafe_allow_html=True)
    ...
    if emp["name"]:
        ...
        st.markdown(f"<small>📅 {emp['quarter']}</small>", unsafe_allow_html=True)
    else:
        st.markdown("<small style='opacity:0.6'>Complete Employee Setup to begin</small>", unsafe_allow_html=True)
    ...
    for score, label in SCORE_LABELS.items():
        ...
        st.markdown(f"<small><span style='color:{color};font-weight:600;'>{score}</span> — {label}</small>",
                    unsafe_allow_html=True)
...
if selected_page == "Overview & Summary":
    st.title('📈 PerformX Tracker')
    st.write('A universal, bias-aware performance tracking system designed for real-world usability and structured evaluation.')
    st.markdown('---')
            ...
            for cat_name, cat_data in CATEGORIES.items():
                ...
                <div style='margin-bottom:0.7rem;'>
                  <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:0.25rem;'>
                    <span style='font-size:0.88rem;font-weight:500;color:#1F3864;'>{cat_name}</span>
                    <span style='font-size:0.88rem;font-weight:700;color:{bar_color};'>{score_disp} <span style='font-weight:400;color:#64748B;font-size:0.8rem;'>— {label}</span></span>
                  </div>
        ...
        with col_s:
            ...
            for cat, score in sorted_cats[:3]:
                st.markdown(f"""<div style='display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem;'>
                    ...
                    <span style='font-size:0.88rem;color:#374151;'>{cat}</span>
    ...
    for i, (label, val) in enumerate(fields):
        with cols[i % 3]:
            st.markdown(f"<div class='field-label'>{label}</div><div style='font-size:0.95rem;color:#60A5FA;font-weight:500;'>{val}</div><br>",
                        unsafe_allow_html=True)
        ...
        if submitted:
            if not name or not role:
                ...
                st.session_state.employee_info.update({
                    ...
                    "manager": manager, "quarter": quarter, "date": review_date
                })
                st.success("✅ Employee information saved. You can now proceed to Category Input.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-subtitle">Enter metric targets, actual values, and scores for each performance category. Category scores auto-calculate from metric scores.</div>',
                unsafe_allow_html=True)

    if not st.session_state.employee_info.get("name"):
        st.markdown('<div class="warning-box">⚠️ Please complete <strong>Employee Setup</strong> before entering scores.</div>',
                    unsafe_allow_html=True)
    ...
    for tab, cat_name in zip(cat_tabs, cat_names):
        ...
        with tab:
            ...
            with col_def:
                st.markdown(f"<div style='font-size:1.05rem;font-weight:700;color:#F1F5F9;margin-bottom:0.3rem;'>{cat_name}</div>",
                            unsafe_allow_html=True)
            ...
            # Bias warning
            st.markdown(f'<div class="warning-box">⚡ <strong>Bias Risk ({cat_data["bias_risk"]}):</strong> {cat_data["bias_note"]}</div>',
                        unsafe_allow_html=True)
                ...
                with st.container():
                    ...
                    <div class='category-card'>
                        <div style='display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem;'>
                            <span style='font-weight:600;color:#1F3864;font-size:0.95rem;'>{metric['name']}</span>
                            {type_badge(metric['type'])}
                        </div>
                        <div style='font-size:0.83rem;color:#374151;margin-bottom:0.2rem;'>{metric['description']}</div>
                        <div style='font-size:0.8rem;color:#64748B;font-style:italic;'>Measurement logic: {metric['logic']}</div>
                        <div style='font-size:0.8rem;color:#64748B;margin-top:0.2rem;'>Data source: {metric['source']}</div>
                    </div>
                    """, unsafe_allow_html=True)

                    col_t, col_a, col_s = st.columns([1.2, 1.2, 0.8])
                    with col_t:
                        st.markdown('<div class="field-label">Quarterly Target / Benchmark</div>', unsafe_allow_html=True)
                        target = st.text_input(
                            "target", key=f"{key_prefix}__target",
                            value=st.session_state.metric_targets.get(key_prefix, ""),
                            placeholder="e.g. ≥90% on-time, ≤1 revision",
                            label_visibility="collapsed"
                        )
                        st.session_state.metric_targets[key_prefix] = target

                    with col_a:
                        st.markdown('<div class="field-label">Actual Value This Quarter</div>', unsafe_allow_html=True)
                        actual = st.text_input(
                            "actual", key=f"{key_prefix}__actual",
                            value=st.session_state.metric_actuals.get(key_prefix, ""),
                            placeholder="e.g. 87%, 2 revisions",
                            label_visibility="collapsed"
                        )
                        st.session_state.metric_actuals[key_prefix] = actual

                    with col_s:
                        st.markdown('<div class="field-label">Score (1–5)</div>', unsafe_allow_html=True)
                    ...
                    st.markdown('<div class="field-label" style="margin-top:0.5rem;">Evidence Notes</div>', unsafe_allow_html=True)
                    evidence = st.text_area(
                        "evidence", key=f"{key_prefix}__evidence",
                        value=st.session_state.metric_evidence.get(key_prefix, ""),
                        placeholder="Cite specific examples, data points, or decisions that support this score...",
                        height=80,
                        label_visibility="collapsed"
                    )
                    st.session_state.metric_evidence[key_prefix] = evidence
                    st.markdown('<div class="field-hint" style="margin-top:-0.4rem;">Tip: Reference a specific deliverable, date, or outcome — not a general description.</div>',
                                unsafe_allow_html=True)
                    st.markdown("<br>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="info-box">💡 <strong>What makes a strong response:</strong> Use action verbs. Reference a specific situation from this quarter. State what you did, why, and what the outcome was. Avoid phrases like "I generally..." or "I try to..." — describe what actually happened.</div>',
                unsafe_allow_html=True)
    ...
    for tab, cat_name in zip(qual_tabs, cat_names):
        ...
        with tab:
            ...
            score = get_category_score(cat_name)
            st.markdown(f"""
            <div style='background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:0.8rem 1.2rem;margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center;'>
                <div>
                    <span style='font-weight:600;color:#1F3864;'>{cat_name}</span>
                    <span style='font-size:0.82rem;color:#64748B;margin-left:0.8rem;'>{cat_data["definition"]}</span>
                </div>
